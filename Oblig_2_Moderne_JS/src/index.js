...
try {
  (function Demo() {
    ...
    test('should work as expected', () => {         expect(myDemo).toBe("This is a demo");  })
...
//   })
  ...
  (function UseLetOrConst() {
    ...
    test('Use let (or const) instead of var to make the function succeed.', () => { expect(x).toBe(1);})
  ...
  /// Template strings ///
	...
	(function UseTemplateStrings1() {
		...
		test( " Rewrite the line below to use template literals.", () => {expect(greeting).toBe('Hello World! 3 times.');  });
  ...
  /// functions default parameters ///
	...
	(function UseDefaultParameters() {
		...
		test( 'Correct the syntax errors in the function.',  ()=> {    expect(hello()).toBe('Hello World!');
  ...
  (function UseParameterDestructuring1() {
    ...
    function f([name, age]) {
	  test( 'UseParameterDestructuring1', ()=> { 
      	expect(name).toBe('Oslo');
      	expect(age).toBe(985);
	  })
    }
